...
function KycUI() {
  ...
  useEffect(() => {
    ...
    const fetchAwaitings = async () => {
      const res = await getAwaiting();
      if (res?.status) {
            ...
            {closedDeals?.length > 0 ? (
              closedDeals.map((p, i) => (
                <div key={i}>
                  ...
                  >
                    <h6
                      className="text-gray-800 text-xs my-1 flex items-center justify-between"
                      style={{ fontFamily: "Opensans-bold" }}
                    >
                      {p?.property_code}
                      {p?.meeting_status && (
                        <p>
                          Meeting Status:{" "}
                          <span className="px-2 rounded-lg bg-red-400">
                            {p?.meeting_status}
                          </span>
                        </p>
                      )}
              ...
              <p className="text-red-500 p-3">No awaiting deals found!</p>
            )}
          </div>
        </div>
      )}
