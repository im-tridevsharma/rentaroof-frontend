...
function NotificationUI() {
  ...
  const delNotification = async (id) => {
    if (id) {
      setIsLoading(true);
      const res = await delUserNotification(id);
      if (res?.status) {
  ...
  const markSeen = async (id) => {
    if (id) {
      setIsLoading(true);
      const res = await seenUserNotification(id);
      if (res?.status) {
  ...
  return (
    <>
      {isLoading && <Loader />}
      <div
        className="flex flex-col p-5 mx-4 bg-white border-gray-200 shadow-sm rounded-md"
