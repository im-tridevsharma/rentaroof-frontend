...
function Add() {
  ...
  const submitData = async (data) => {
    const response = await addPreference(data);
    if (response?.status) {
      setIsAdded(true);
      setVErrors(false);
      ...
      {isAdded && (
        <div className="w-full mb-4">
          <Alert
            ...
            raised
          >
            New Preference added successfully.
          </Alert>
        </div>
      )}
      <div className="bg-white dark:bg-gray-800 flex flex-col px-2 py-3 rounded-lg border-gray-100 dark:border-gray-900 border-2">
